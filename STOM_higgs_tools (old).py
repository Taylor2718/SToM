...
plt.plot(Siuuu,B(Siuuu,fit_Back[0],fit_Back[1]),label='Background', color = 'red')
plt.plot(av_bin_edges[0:30],bin_heights,'x', label = 'Data')
plt.legend()
plt.grid()
#%%CHI (BACKGROUND)
back_chi=get_B_chi(vals,(104,119.29962839849368),9,fit_Back[0],fit_Back[1]) #Chi squared test for background fit against data, where historgram upper bound determined using vals list
global_chi=get_B_chi(vals, (104, 155),30,fit_Back[0],fit_Back[1]) #Chi sqaured test for total range of masses
#%%HYPOTHESIS TESTING
sig_chi=[]
for i in range(0,10000):
    vals_rep=generate_data()
    global_chi=get_B_chi(vals_rep, (104, 155),30,fit_Back[0],fit_Back[1])
    sig_chi.append(global_chi)
bin_heights_chi, bin_edges_chi, patches_chi = plt.hist(sig_chi, range = [min(sig_chi),max(sig_chi)], bins = 30)
plt.xlabel('x')
plt.ylabel('pdf')
